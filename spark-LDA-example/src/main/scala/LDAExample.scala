...
class LDAExample(sc: SparkContext, spark: SparkSession) {
  ...
  def run(params: Params): Unit = {
    ...
    println("corpus.toDF().show")
    corpus.toDF().show(10)
    vocabArray.take(3).foreach(println)
    ...
    if (ldaModel.isInstanceOf[DistributedLDAModel]) {
      ...
      distLDAModel.topicAssignments.toDF("docId", "sortedTerms", "sortedTopics").show(5)

      distLDAModel.topTopicsPerDocument(5).toDF("docId", "sortedTerms", "sortedTopics").show(5, truncate = false)

      corpus.filter(_._1 == 14).toDF().show()
  ...
  def preprocess(
    ...
    println("read test file with path:", paths)
    ...
    println("读取文件的doc列：rdd.toDF(\"docs\")")
    df.show(3)
