...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new scopt.OptionParser[Config](this.getClass().toString()){
      ...
      opt[Int]('x',"minprogress") action {(x,c)=> c.copy(minProgress=x)} text("Number of vertices that must change communities for the algorithm to consider progress. default=2000")
