...
object LouvainCore {
  ...
  def louvain(sc: SparkContext, graph: Graph[VertexState, Long], minProgress: Int = 1, progressCounter: Int = 1): (Double, Graph[VertexState, Long], Int) = {
    ...
    do {
      count += 1
      println("第 " + count + " 次循环开始...")
      printEdgetriplets(graph)
      printLouvain(graph)
      even = !even
      ...
      println("updated 值：" + updated)

      println("第 " + count + " 次循环结束")

    } while (stop <= progressCounter && (even || (updated > 0 && count < maxIter)))
    println("\n结束战斗，Completed in " + count + " cycles")
    ...
    val newVerts = louvainGraph.vertices.innerJoin(msgRDD)((vid, vdata, msgs) => {
      ...
      println(s"节点: $vid 在社区: $community  ( 最佳Q: $q = ($k_i_in / $M) -  ( ($sigmaTot * $k_i) / math.pow($M, 2) )")
      if (q < 0) 0 else q
    })
  ...
  private def louvainVertJoin(louvainGraph: Graph[VertexState, Long], msgRDD: VertexRDD[Map[(Long, Long), Long]], totalEdgeWeight: Broadcast[Long], even: Boolean) = {
    louvainGraph.vertices.innerJoin(msgRDD)((vid, vdata, msgs) => {
      ...
      msgs.foreach({ case ((communityId, sigmaTotal), communityEdgeWeight) =>
        ...
        //  println("计算社区:" + communityId +  ", 之前的最好社区是：" + bestCommunity)
        // println("community: " + communityId + "\n sigma:" + sigmaTotal + "\n edgeweight:" + communityEdgeWeight + "\nq:" + deltaQ)
        if (deltaQ > maxDeltaQ || (deltaQ > 0 && (deltaQ == maxDeltaQ && communityId > bestCommunity))) {
          println("  计算社区:" + communityId + ", 牛逼，Best: " + bestCommunity + " -> " + communityId)
          maxDeltaQ = deltaQ
          bestCommunity = communityId
          bestSigmaTot = sigmaTotal
        } else {
          println("  计算社区:" + communityId + ", 普通，Best: " + bestCommunity)
        }
      })
      // only allow changes from low to high communities on even cyces and high to low on odd cycles
      if (vdata.community != bestCommunity && ((even && vdata.community > bestCommunity) || (!even && vdata.community < bestCommunity))) {
        println("所以节点: " + vid + " 更新社区: " + vdata.community + " -> " + bestCommunity)
        vdata.community = bestCommunity
        vdata.communitySigmaTot = bestSigmaTot
        vdata.changed = true
      } else {
        println("所以节点: " + vid + " 保持社区: " + vdata.community )
        vdata.changed = false
      }
      vdata
    })
  ...
  // debug printing
  private def printLouvain(graph: Graph[VertexState, Long]): Unit = {
  ...
  // debug printing
  private def printEdgetriplets(graph: Graph[VertexState, Long]): Unit = {
